...
class NFT(db.Model):
    ...
    owner_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)  # Делаем nullable
    date_created = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    assigned = db.Column(db.Boolean, default=False)  # Добавляем флаг назначения
...
def create_nft():
    ...
    if request.method == 'POST':
        ...
        owner_id = request.form.get('owner_id')  # Используем get() так как поле может отсутствовать
        ...
        if 'image' in request.files:
            ...
            if image.filename != '':
                ...
                new_nft = NFT(
                    ...
                    assigned=bool(owner_id)  # Устанавливаем флаг назначения
                db.session.add(new_nft)
                db.session.commit()
                
                flash('NFT успешно создан!', 'success')
                return redirect(url_for('unassigned_nfts'))  # Перенаправляем на страницу неназначенных NFT
    
    students = User.query.filter_by(role='student').all() if session['role'] == 'teacher' else []
